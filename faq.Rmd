...
rsvp_more <- rsvp_raw %>%
  ...
  drop_na(name) %>% 
  select(- c(contains("July"), num_attends, xval, timestamp, email, 
  ...
  filter(activity != "NA") %>% 
  mutate(act_val = 1) %>%
  separate(nights, sep = ",", into = c("Camp Friday", "Camp Saturday")) %>%  
  pivot_wider(names_from = activity, values_from = act_val) %>% 
  select(- contains("Ceremony"), - contains("Reception"))
...
mismatches <- comb_rsvp %>% 
  filter(is.na(will_attend) | will_attend == "Yes!!!",
...
comb_emails = comb_rsvp %>% filter(RSVP > 0) %>% pull(email) 
...
word_counts_df <- notes_df %>%
  ...
  ungroup() %>% 
  mutate(att_name = gsub('@Gmail.com|\"', "", att_name),
         att_name = case_when(
